...
def _make_provider(config: Config):
    ...
    # xAI: direct to Responses API (no LiteLLM wrapper)
    if provider_name == "xai":
        api_key = p.api_key if p else os.environ.get("XAI_API_KEY", "")
        model_name = model.removeprefix("xai/") if model.startswith("xai/") else model
        from nanobot.providers.xai_responses_provider import XAIResponsesProvider

        return XAIResponsesProvider(api_key=api_key, default_model=model_name)
